...
try{const statements=[env.DB.prepare('INSERT INTO reports (key,payload,received_at) VALUES (?,?,?) ON CONFLICT(key) DO UPDATE SET payload=excluded.payload,received_at=excluded.received_at').bind(key,serialized,now)];if(type==='node'){const valid=payload.gpus.filter(g=>!g.collection_error&&g.gpu_utilization!==null&&g.gpu_utilization>=0&&g.gpu_utilization<=100);const minute=Math.floor(now/60000)*60000;statements.push(env.DB.prepare('INSERT INTO gpu_history (key,node,minute,utilization,gpu_count) VALUES (?,?,?,?,?) ON CONFLICT(key) DO UPDATE SET utilization=excluded.utilization,gpu_count=excluded.gpu_count').bind(`${payload.server_name}:${minute}`,payload.server_name,minute,valid.length?valid.reduce((s,g)=>s+g.gpu_utilization,0)/valid.length:null,valid.length));statements.push(env.DB.prepare('DELETE FROM gpu_history WHERE minute < ?').bind(now-86400000));}await env.DB.batch(statements);return json({ok:true,received_at:now,...(type==='slurm'?{accounting_status:'not_enabled'}:{})});}catch(error){console.error('report storage failed',error.message);return json({error:'storage_unavailable'},503);}}
...
try{const reports=await env.DB.prepare('SELECT key,payload,received_at FROM reports ORDER BY key').all();const history=await env.DB.prepare('SELECT minute,SUM(utilization*gpu_count)/NULLIF(SUM(gpu_count),0) AS utilization,SUM(gpu_count) AS gpu_count FROM gpu_history WHERE minute >= ? GROUP BY minute ORDER BY minute').bind(Date.now()-86400000).all();const data={serverTime:Date.now(),slurm:null,nodes:[],history:history.results};for(const row of reports.results){const item={receivedAt:row.received_at,data:JSON.parse(row.payload)};if(row.key==='slurm')data.slurm=item;else if(row.key.startsWith('node:'))data.nodes.push(item);}return json(data);}catch(error){console.error('snapshot unavailable',error.message);return json({error:'storage_unavailable'},503);}}
