...
- Server1~4의 GPU별 Job ID·사용자·작업명
...
연구실 에이전트가 이미 전송하는 `psutil.disk_usage(...).free`는 유지합니다. 클라우드 브리지는 `shutil.disk_usage(...).free`를 사용합니다. Linux에서 이 값은 일반 사용자가 사용할 수 있는 블록(`f_bavail`)을 사용하며, 디스크 조회가 실패하면 해당 용량을 `null`로 전송합니다.

## 운영 종료된 노드

Baro(`baro-1`)는 2026-09-27 대여 종료에 따라 대시보드에서 제외했습니다. API는 저장된 Baro 보고를 스냅샷에서 제외하고, 인증된 후속 node/cloud-gpu 보고에도 `410 node_retired`를 반환해 다시 등록되지 않게 합니다. Server1~4와 Slurm 수집에는 영향을 주지 않습니다. 대여 종료된 서버에 접속하거나 원격 파일을 변경할 필요는 없습니다.
...
검증: 전송 인증, 공개/비공개 조회 정책, 입력 오류, 빈 작업 목록, 0과 null의 구분, 최신 보고 덮어쓰기, 이력 접근 제거, 한도 초과 응답과 UTC 재설정 경계, 수집기 재시도 간격을 검사합니다. 브라우저에서는 한도 안내, 수동 재시도, 30초 조회/3분 유효 기간, 이전 데이터의 지연 표시와 모바일 레이아웃을 확인합니다.
