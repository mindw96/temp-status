...
배포가 성공하면 Render 서비스 화면에 `https://...onrender.com` 주소가 표시됩니다. 홈페이지가 열리고 `/healthz`가 성공하면 서버 실행은 정상입니다. **수집 목적지를 바꾸기 전까지는 GPU 데이터가 없는 것이 정상입니다.**
...
기존 환경에서는 새 Render 주소를 Codex에 전달하면 전송 목적지 변경과 다섯 노드·Slurm 수신 검증을 이어서 수행할 수 있습니다. 토큰은 다시 전달할 필요가 없습니다.
